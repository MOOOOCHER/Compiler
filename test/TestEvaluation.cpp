...
static auto setup(std::vector<double> args, const std::string_view& input){
    ...
    return evaluator.evaluateFunction(args,*semantic.analyzeFunction(std::vector<double>(),*parseNode));
}
...
TEST(TestEvaluation, DeadCodeElimination){
    auto result = setup(std::vector<double>(),"VAR a,b,c,d,e,f,z,g,i,j,ab,aaa ; BEGIN ab := 1 ;RETURN ab;ab := 1 ;ab := 1 ;ab := 1  END.");
    EXPECT_EQ(result.value(), 1);
    result = setup(std::vector<double>(),"VAR a, b; BEGIN a := 1; b := 1; RETURN (a+b * 2/(4+4));a := 1; b := 1;a := 1; b := 1 END.");
    EXPECT_EQ(result.value(), 1.25);
    result = setup(std::vector<double>(),"VAR a, b; BEGIN a := 1; b := 1; RETURN a *(-b + 55 * (1-(-1)) ) ;a := 1; b := 1;a := 1; b := 1 END.");
    EXPECT_EQ(result.value(), 109);
}
TEST(TestEvaluation, ConstantPropagation){
    auto result = setup(std::vector<double>(),"VAR a, b; BEGIN a := 1; b := 1; RETURN (a+b * 2/(4+4));a := 1; b := 1;a := 1; b := 1 END.");
    EXPECT_EQ(result.value(), 1.25);
    result = setup(std::vector<double>(),"VAR a, b; BEGIN a := 1; b := 1; RETURN a *(-b + 55 * (1-(-1)) ) ;a := 1; b := 1;a := 1; b := 1 END.");
    EXPECT_EQ(result.value(), 109);
}
